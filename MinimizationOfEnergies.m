function [u, offset_map] = MinimizationOfEnergies(u_0, M, sigma2, tolerance, lambda, half_size_patch, median, average, poisson)
    ...
    while cnt <= 10
        ...
        if cnt == 1
            offset_map = NewPatchMatch (u_1, u_1 .* (1 - M), M, half_size_patch, 1, error, lambda);%, lambda, M, sigma2, median, average, poisson,1);
        else
            offset_map = NewPatchMatch (u_1, u_1 .* (1 - M), M, half_size_patch, 1, error, lambda, offset_map);
        end
