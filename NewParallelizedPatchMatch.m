function NNF = NewParallelizedPatchMatch(A, B, mask, half_patch, iterations, error, lambda, NNF)
tic;
    A = double(A);
    B = double(B);
    ...
    while k <= iterations 
        ...
        parfor N = 1 : nb_cores
            NNF_chunk{N} = NNF;
            NNF_mask_chunk{N} = false(size(NNF));
            ...
            for n = x_start : x_change : x_end
                ...
                % Also activating the updated pixel for the later update of
                % the real NNF.
                NNF_chunk{N} = UpdateNNF(NNF_chunk{N}, i, j, best_x, best_y, best_guess);                       
                NNF_mask_chunk{N}(i, j, :) = true;
            end
        end
        
        % Updating the actual NNF using the activated parallel NNF pixels.
        for i = 1 : nb_cores            
            NNF(NNF_mask_chunk{i} == true) = NNF_chunk{i}(NNF_mask_chunk{i} == true);
        end
        k = k + 1;
    end
toc;
