function uz = gradient_conjugue (u0, epsilon, lambda, kz, fz, vx,vy, nb_iter,i)
uz = u0;
...
for i=1:nb_iter
   ...
   %tmp(:,:,2) = p_t(:,:,2) * Ap(:,:,2);
   %tmp(:,:,3) = p_t(:,:,3) * Ap(:,:,3);
   alpha(:,:,1) = (r_t(:,:,1)*r(:,:,1))./(p_t(:,:,1)*Ap(:,:,1));
   alpha(:,:,2) = (r_t(:,:,2)*r(:,:,2))./(p_t(:,:,2)*Ap(:,:,2));
   alpha(:,:,3) = (r_t(:,:,3)*r(:,:,3))./(p_t(:,:,3)*Ap(:,:,3));
