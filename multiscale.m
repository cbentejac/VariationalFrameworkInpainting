...
for l = L:-1:1
    ...
    if (l == L)
       for c=1:channel
           u_l(:,:,c)= pyramid_image{l}(:,:,c).*~M_l+sum(sum(u0(:,:,c).*~Mask))/sum(~Mask).*M_l; %1->inpainting domain
       end
    else
        %Upsample correspondence phi using NN interpolation and scale by r
        %need to make sure the every entry of our resized correspondance
        %map is also a integer, so Nearest Neighbour Interpolation is used(using ONLY
        %the existing values to create new contents). 
        phi_l = r*imresize(phi_l,r,'nearest'); 
        
        %initialize initial condition u_l(u_0^s in the paper) at level l using image update step with
        %resized phi_l
        u_hat_l = pyramid_image{l}.*M_l;
        u_l = image_update(phi_l, u_hat_l, M_l, size_patch, sigma2);
    end
    
    %solve with algorithm minimization energy
